ENCHANTS = {
    "consumables": {
        "风语·絮泊磨石（疗伤） 疗伤成效提升1194点": {
            "therapy_power_base": 1194
        },
        "风语·坠宵磨石（内伤） 内功攻击提升746点": {
            "magical_attack_power_base": 746
        },
        "风语·瀑沙磨石（外伤） 外功攻击提升669点": {
            "physical_attack_power_base": 669
        },
        "风语·絮泊熔锭（疗伤） 疗伤成效提升2388点": {
            "therapy_power_base": 2388
        },
        "风语·坠宵熔锭（内攻） 内功攻击提升1491点": {
            "magical_attack_power_base": 1491
        },
        "风语·瀑沙熔锭（外攻） 外功攻击提升1337点": {
            "physical_attack_power_base": 1337
        },
        "风语·絮泊磨石（疗伤） 疗伤成效提升1020点": {
            "therapy_power_base": 1020
        },
        "风语·坠宵磨石（内伤） 内功攻击提升637点": {
            "magical_attack_power_base": 637
        },
        "风语·瀑沙磨石（外伤） 外功攻击提升571点": {
            "physical_attack_power_base": 571
        },
        "风语·絮泊熔锭（疗伤） 疗伤成效提升2040点": {
            "therapy_power_base": 2040
        },
        "风语·坠宵熔锭（内攻） 内功攻击提升1273点": {
            "magical_attack_power_base": 1273
        },
        "风语·瀑沙熔锭（外攻） 外功攻击提升1142点": {
            "physical_attack_power_base": 1142
        }
    },
    "tertiary_weapon": {
        "暗影·荆岫·暗器（加速） 加速提升5739点": {
            "id": 16420,
            "name": "暗影·荆岫·暗器（加速）",
            "score": 5052,
            "attributes": {
                "haste_base": 5739
            }
        },
        "暗影·荆岫·暗器（内破） 内功破防等级提升5739点": {
            "id": 16419,
            "name": "暗影·荆岫·暗器（内破）",
            "score": 5052,
            "attributes": {
                "magical_overcome_base": 5739
            }
        },
        "暗影·荆岫·暗器（外破） 外功破防等级提升5739点": {
            "id": 16418,
            "name": "暗影·荆岫·暗器（外破）",
            "score": 5052,
            "attributes": {
                "physical_overcome_base": 5739
            }
        },
        "暗影·荆岫·暗器（身法） 身法提升736点": {
            "id": 16417,
            "name": "暗影·荆岫·暗器（身法）",
            "score": 5052,
            "attributes": {
                "agility_base": 736
            }
        },
        "暗影·荆岫·暗器（元气） 元气提升736点": {
            "id": 16416,
            "name": "暗影·荆岫·暗器（元气）",
            "score": 5052,
            "attributes": {
                "spunk_base": 736
            }
        },
        "暗影·荆岫·暗器（力道） 力道提升736点": {
            "id": 16415,
            "name": "暗影·荆岫·暗器（力道）",
            "score": 5052,
            "attributes": {
                "strength_base": 736
            }
        },
        "暗影·荆岫·暗器（根骨） 根骨提升736点": {
            "id": 16414,
            "name": "暗影·荆岫·暗器（根骨）",
            "score": 5052,
            "attributes": {
                "spirit_base": 736
            }
        },
        "暗影·白虹·暗器（加速） 加速提升5739点": {
            "id": 16403,
            "name": "暗影·白虹·暗器（加速）",
            "score": 5052,
            "attributes": {
                "haste_base": 5739
            }
        },
        "暗影·白虹·暗器（内破） 内功破防等级提升5739点": {
            "id": 16402,
            "name": "暗影·白虹·暗器（内破）",
            "score": 5052,
            "attributes": {
                "magical_overcome_base": 5739
            }
        },
        "暗影·白虹·暗器（外破） 外功破防等级提升5739点": {
            "id": 16401,
            "name": "暗影·白虹·暗器（外破）",
            "score": 5052,
            "attributes": {
                "physical_overcome_base": 5739
            }
        },
        "暗影·白虹·暗器（身法） 身法提升736点": {
            "id": 16400,
            "name": "暗影·白虹·暗器（身法）",
            "score": 5052,
            "attributes": {
                "agility_base": 736
            }
        },
        "暗影·白虹·暗器（元气） 元气提升736点": {
            "id": 16399,
            "name": "暗影·白虹·暗器（元气）",
            "score": 5052,
            "attributes": {
                "spunk_base": 736
            }
        },
        "暗影·白虹·暗器（力道） 力道提升736点": {
            "id": 16398,
            "name": "暗影·白虹·暗器（力道）",
            "score": 5052,
            "attributes": {
                "strength_base": 736
            }
        },
        "暗影·白虹·暗器（根骨） 根骨提升736点": {
            "id": 16397,
            "name": "暗影·白虹·暗器（根骨）",
            "score": 5052,
            "attributes": {
                "spirit_base": 736
            }
        },
        "山海·荆岫·暗器（加速） 加速提升4902点": {
            "id": 16209,
            "name": "山海·荆岫·暗器（加速）",
            "score": 4316,
            "attributes": {
                "haste_base": 4902
            }
        },
        "山海·荆岫·暗器（内破） 内功破防等级提升4902点": {
            "id": 16208,
            "name": "山海·荆岫·暗器（内破）",
            "score": 4316,
            "attributes": {
                "magical_overcome_base": 4902
            }
        },
        "山海·荆岫·暗器（外破） 外功破防等级提升4902点": {
            "id": 16207,
            "name": "山海·荆岫·暗器（外破）",
            "score": 4316,
            "attributes": {
                "physical_overcome_base": 4902
            }
        },
        "山海·荆岫·暗器（身法） 身法提升628点": {
            "id": 16206,
            "name": "山海·荆岫·暗器（身法）",
            "score": 4316,
            "attributes": {
                "agility_base": 628
            }
        },
        "山海·荆岫·暗器（元气） 元气提升628点": {
            "id": 16205,
            "name": "山海·荆岫·暗器（元气）",
            "score": 4316,
            "attributes": {
                "spunk_base": 628
            }
        },
        "山海·荆岫·暗器（力道） 力道提升628点": {
            "id": 16204,
            "name": "山海·荆岫·暗器（力道）",
            "score": 4316,
            "attributes": {
                "strength_base": 628
            }
        },
        "山海·荆岫·暗器（根骨） 根骨提升628点": {
            "id": 16203,
            "name": "山海·荆岫·暗器（根骨）",
            "score": 4316,
            "attributes": {
                "spirit_base": 628
            }
        },
        "山海·白虹·暗器（加速） 加速提升4902点": {
            "id": 16192,
            "name": "山海·白虹·暗器（加速）",
            "score": 4316,
            "attributes": {
                "haste_base": 4902
            }
        },
        "山海·白虹·暗器（内破） 内功破防等级提升4902点": {
            "id": 16191,
            "name": "山海·白虹·暗器（内破）",
            "score": 4316,
            "attributes": {
                "magical_overcome_base": 4902
            }
        },
        "山海·白虹·暗器（外破） 外功破防等级提升4902点": {
            "id": 16190,
            "name": "山海·白虹·暗器（外破）",
            "score": 4316,
            "attributes": {
                "physical_overcome_base": 4902
            }
        },
        "山海·白虹·暗器（身法） 身法提升628点": {
            "id": 16189,
            "name": "山海·白虹·暗器（身法）",
            "score": 4316,
            "attributes": {
                "agility_base": 628
            }
        },
        "山海·白虹·暗器（元气） 元气提升628点": {
            "id": 16188,
            "name": "山海·白虹·暗器（元气）",
            "score": 4316,
            "attributes": {
                "spunk_base": 628
            }
        },
        "山海·白虹·暗器（力道） 力道提升628点": {
            "id": 16187,
            "name": "山海·白虹·暗器（力道）",
            "score": 4316,
            "attributes": {
                "strength_base": 628
            }
        },
        "山海·白虹·暗器（根骨） 根骨提升628点": {
            "id": 16186,
            "name": "山海·白虹·暗器（根骨）",
            "score": 4316,
            "attributes": {
                "spirit_base": 628
            }
        },
        "阆风仙玉·暗器（加速） 加速提升4099点": {
            "id": 16438,
            "name": "阆风仙玉·暗器（加速）",
            "score": 3907,
            "attributes": {
                "haste_base": 4099
            }
        },
        "阆风仙玉·暗器（内破） 内功破防等级提升4099点": {
            "id": 16437,
            "name": "阆风仙玉·暗器（内破）",
            "score": 3907,
            "attributes": {
                "magical_overcome_base": 4099
            }
        },
        "阆风仙玉·暗器（外破） 外功破防等级提升4099点": {
            "id": 16436,
            "name": "阆风仙玉·暗器（外破）",
            "score": 3907,
            "attributes": {
                "physical_overcome_base": 4099
            }
        },
        "阆风仙玉·暗器（身法） 身法提升526点": {
            "id": 16435,
            "name": "阆风仙玉·暗器（身法）",
            "score": 3907,
            "attributes": {
                "agility_base": 526
            }
        },
        "阆风仙玉·暗器（元气） 元气提升526点": {
            "id": 16434,
            "name": "阆风仙玉·暗器（元气）",
            "score": 3907,
            "attributes": {
                "spunk_base": 526
            }
        },
        "阆风仙玉·暗器（力道） 力道提升526点": {
            "id": 16433,
            "name": "阆风仙玉·暗器（力道）",
            "score": 3907,
            "attributes": {
                "strength_base": 526
            }
        },
        "阆风仙玉·暗器（根骨） 根骨提升526点": {
            "id": 16432,
            "name": "阆风仙玉·暗器（根骨）",
            "score": 3907,
            "attributes": {
                "spirit_base": 526
            }
        },
        "昆仑玄石·暗器（加速） 加速提升3279点": {
            "id": 16383,
            "name": "昆仑玄石·暗器（加速）",
            "score": 3368,
            "attributes": {
                "haste_base": 3279
            }
        },
        "昆仑玄石·暗器（内破） 内功破防等级提升3279点": {
            "id": 16382,
            "name": "昆仑玄石·暗器（内破）",
            "score": 3368,
            "attributes": {
                "magical_overcome_base": 3279
            }
        },
        "昆仑玄石·暗器（外破） 外功破防等级提升3279点": {
            "id": 16381,
            "name": "昆仑玄石·暗器（外破）",
            "score": 3368,
            "attributes": {
                "physical_overcome_base": 3279
            }
        },
        "昆仑玄石·暗器（身法） 身法提升420点": {
            "id": 16380,
            "name": "昆仑玄石·暗器（身法）",
            "score": 3368,
            "attributes": {
                "agility_base": 420
            }
        },
        "昆仑玄石·暗器（元气） 元气提升420点": {
            "id": 16379,
            "name": "昆仑玄石·暗器（元气）",
            "score": 3368,
            "attributes": {
                "spunk_base": 420
            }
        },
        "昆仑玄石·暗器（力道） 力道提升420点": {
            "id": 16378,
            "name": "昆仑玄石·暗器（力道）",
            "score": 3368,
            "attributes": {
                "strength_base": 420
            }
        },
        "昆仑玄石·暗器（根骨） 根骨提升420点": {
            "id": 16377,
            "name": "昆仑玄石·暗器（根骨）",
            "score": 3368,
            "attributes": {
                "spirit_base": 420
            }
        },
        "昆仑玄石·暗器（加速） 加速提升2801点": {
            "id": 16157,
            "name": "昆仑玄石·暗器（加速）",
            "score": 2877,
            "attributes": {
                "haste_base": 2801
            }
        },
        "昆仑玄石·暗器（内破） 内功破防等级提升2801点": {
            "id": 16156,
            "name": "昆仑玄石·暗器（内破）",
            "score": 2877,
            "attributes": {
                "magical_overcome_base": 2801
            }
        },
        "昆仑玄石·暗器（外破） 外功破防等级提升2801点": {
            "id": 16155,
            "name": "昆仑玄石·暗器（外破）",
            "score": 2877,
            "attributes": {
                "physical_overcome_base": 2801
            }
        },
        "昆仑玄石·暗器（身法） 身法提升359点": {
            "id": 16154,
            "name": "昆仑玄石·暗器（身法）",
            "score": 2877,
            "attributes": {
                "agility_base": 359
            }
        },
        "昆仑玄石·暗器（元气） 元气提升359点": {
            "id": 16153,
            "name": "昆仑玄石·暗器（元气）",
            "score": 2877,
            "attributes": {
                "spunk_base": 359
            }
        },
        "昆仑玄石·暗器（力道） 力道提升359点": {
            "id": 16152,
            "name": "昆仑玄石·暗器（力道）",
            "score": 2877,
            "attributes": {
                "strength_base": 359
            }
        },
        "昆仑玄石·暗器（根骨） 根骨提升359点": {
            "id": 16151,
            "name": "昆仑玄石·暗器（根骨）",
            "score": 2877,
            "attributes": {
                "spirit_base": 359
            }
        }
    },
    "ring": {
        "暗影·荆岫·戒指（内攻） 内功攻击提升1739点": {
            "id": 16413,
            "name": "暗影·荆岫·戒指（内攻）",
            "score": 5052,
            "attributes": {
                "magical_attack_power_base": 1739
            }
        },
        "暗影·荆岫·戒指（外攻） 外功攻击提升1559点": {
            "id": 16412,
            "name": "暗影·荆岫·戒指（外攻）",
            "score": 5052,
            "attributes": {
                "physical_attack_power_base": 1559
            }
        },
        "暗影·荆岫·戒指（身法） 身法提升736点": {
            "id": 16411,
            "name": "暗影·荆岫·戒指（身法）",
            "score": 5052,
            "attributes": {
                "agility_base": 736
            }
        },
        "暗影·荆岫·戒指（元气） 元气提升736点": {
            "id": 16410,
            "name": "暗影·荆岫·戒指（元气）",
            "score": 5052,
            "attributes": {
                "spunk_base": 736
            }
        },
        "暗影·荆岫·戒指（力道） 力道提升736点": {
            "id": 16409,
            "name": "暗影·荆岫·戒指（力道）",
            "score": 5052,
            "attributes": {
                "strength_base": 736
            }
        },
        "暗影·荆岫·戒指（根骨） 根骨提升736点": {
            "id": 16408,
            "name": "暗影·荆岫·戒指（根骨）",
            "score": 5052,
            "attributes": {
                "spirit_base": 736
            }
        },
        "暗影·白虹·戒指（内攻） 内功攻击提升1739点": {
            "id": 16396,
            "name": "暗影·白虹·戒指（内攻）",
            "score": 5052,
            "attributes": {
                "magical_attack_power_base": 1739
            }
        },
        "暗影·白虹·戒指（外攻） 外功攻击提升1559点": {
            "id": 16395,
            "name": "暗影·白虹·戒指（外攻）",
            "score": 5052,
            "attributes": {
                "physical_attack_power_base": 1559
            }
        },
        "暗影·白虹·戒指（身法） 身法提升736点": {
            "id": 16394,
            "name": "暗影·白虹·戒指（身法）",
            "score": 5052,
            "attributes": {
                "agility_base": 736
            }
        },
        "暗影·白虹·戒指（元气） 元气提升736点": {
            "id": 16393,
            "name": "暗影·白虹·戒指（元气）",
            "score": 5052,
            "attributes": {
                "spunk_base": 736
            }
        },
        "暗影·白虹·戒指（力道） 力道提升736点": {
            "id": 16392,
            "name": "暗影·白虹·戒指（力道）",
            "score": 5052,
            "attributes": {
                "strength_base": 736
            }
        },
        "暗影·白虹·戒指（根骨） 根骨提升736点": {
            "id": 16391,
            "name": "暗影·白虹·戒指（根骨）",
            "score": 5052,
            "attributes": {
                "spirit_base": 736
            }
        },
        "山海·荆岫·戒指（内攻） 内功攻击提升1485点": {
            "id": 16202,
            "name": "山海·荆岫·戒指（内攻）",
            "score": 4316,
            "attributes": {
                "magical_attack_power_base": 1485
            }
        },
        "山海·荆岫·戒指（外攻） 外功攻击提升1332点": {
            "id": 16201,
            "name": "山海·荆岫·戒指（外攻）",
            "score": 4316,
            "attributes": {
                "physical_attack_power_base": 1332
            }
        },
        "山海·荆岫·戒指（身法） 身法提升628点": {
            "id": 16200,
            "name": "山海·荆岫·戒指（身法）",
            "score": 4316,
            "attributes": {
                "agility_base": 628
            }
        },
        "山海·荆岫·戒指（元气） 元气提升628点": {
            "id": 16199,
            "name": "山海·荆岫·戒指（元气）",
            "score": 4316,
            "attributes": {
                "spunk_base": 628
            }
        },
        "山海·荆岫·戒指（力道） 力道提升628点": {
            "id": 16198,
            "name": "山海·荆岫·戒指（力道）",
            "score": 4316,
            "attributes": {
                "strength_base": 628
            }
        },
        "山海·荆岫·戒指（根骨） 根骨提升628点": {
            "id": 16197,
            "name": "山海·荆岫·戒指（根骨）",
            "score": 4316,
            "attributes": {
                "spirit_base": 628
            }
        },
        "山海·白虹·戒指（内攻） 内功攻击提升1485点": {
            "id": 16185,
            "name": "山海·白虹·戒指（内攻）",
            "score": 4316,
            "attributes": {
                "magical_attack_power_base": 1485
            }
        },
        "山海·白虹·戒指（外攻） 外功攻击提升1332点": {
            "id": 16184,
            "name": "山海·白虹·戒指（外攻）",
            "score": 4316,
            "attributes": {
                "physical_attack_power_base": 1332
            }
        },
        "山海·白虹·戒指（身法） 身法提升628点": {
            "id": 16183,
            "name": "山海·白虹·戒指（身法）",
            "score": 4316,
            "attributes": {
                "agility_base": 628
            }
        },
        "山海·白虹·戒指（元气） 元气提升628点": {
            "id": 16182,
            "name": "山海·白虹·戒指（元气）",
            "score": 4316,
            "attributes": {
                "spunk_base": 628
            }
        },
        "山海·白虹·戒指（力道） 力道提升628点": {
            "id": 16181,
            "name": "山海·白虹·戒指（力道）",
            "score": 4316,
            "attributes": {
                "strength_base": 628
            }
        },
        "山海·白虹·戒指（根骨） 根骨提升628点": {
            "id": 16180,
            "name": "山海·白虹·戒指（根骨）",
            "score": 4316,
            "attributes": {
                "spirit_base": 628
            }
        },
        "阆风仙玉·戒指（破招） 破招等级提升4099点": {
            "id": 16431,
            "name": "阆风仙玉·戒指（破招）",
            "score": 3907,
            "attributes": {
                "surplus_base": 4099
            }
        },
        "阆风仙玉·戒指（内伤） 内功攻击提升1242点": {
            "id": 16430,
            "name": "阆风仙玉·戒指（内伤）",
            "score": 3907,
            "attributes": {
                "magical_attack_power_base": 1242
            }
        },
        "阆风仙玉·戒指（外伤） 外功攻击提升1114点": {
            "id": 16429,
            "name": "阆风仙玉·戒指（外伤）",
            "score": 3907,
            "attributes": {
                "physical_attack_power_base": 1114
            }
        },
        "阆风仙玉·戒指（身法） 身法提升526点": {
            "id": 16428,
            "name": "阆风仙玉·戒指（身法）",
            "score": 3907,
            "attributes": {
                "agility_base": 526
            }
        },
        "阆风仙玉·戒指（元气） 元气提升526点": {
            "id": 16427,
            "name": "阆风仙玉·戒指（元气）",
            "score": 3907,
            "attributes": {
                "spunk_base": 526
            }
        },
        "阆风仙玉·戒指（力道） 力道提升526点": {
            "id": 16426,
            "name": "阆风仙玉·戒指（力道）",
            "score": 3907,
            "attributes": {
                "strength_base": 526
            }
        },
        "阆风仙玉·戒指（根骨） 根骨提升526点": {
            "id": 16425,
            "name": "阆风仙玉·戒指（根骨）",
            "score": 3907,
            "attributes": {
                "spirit_base": 526
            }
        },
        "昆仑玄石·戒指（破招） 破招等级提升3279点": {
            "id": 16376,
            "name": "昆仑玄石·戒指（破招）",
            "score": 3368,
            "attributes": {
                "surplus_base": 3279
            }
        },
        "昆仑玄石·戒指（内伤） 内功攻击提升994点": {
            "id": 16375,
            "name": "昆仑玄石·戒指（内伤）",
            "score": 3368,
            "attributes": {
                "magical_attack_power_base": 994
            }
        },
        "昆仑玄石·戒指（外伤） 外功攻击提升891点": {
            "id": 16374,
            "name": "昆仑玄石·戒指（外伤）",
            "score": 3368,
            "attributes": {
                "physical_attack_power_base": 891
            }
        },
        "昆仑玄石·戒指（身法） 身法提升420点": {
            "id": 16373,
            "name": "昆仑玄石·戒指（身法）",
            "score": 3368,
            "attributes": {
                "agility_base": 420
            }
        },
        "昆仑玄石·戒指（元气） 元气提升420点": {
            "id": 16372,
            "name": "昆仑玄石·戒指（元气）",
            "score": 3368,
            "attributes": {
                "spunk_base": 420
            }
        },
        "昆仑玄石·戒指（力道） 力道提升420点": {
            "id": 16371,
            "name": "昆仑玄石·戒指（力道）",
            "score": 3368,
            "attributes": {
                "strength_base": 420
            }
        },
        "昆仑玄石·戒指（根骨） 根骨提升420点": {
            "id": 16370,
            "name": "昆仑玄石·戒指（根骨）",
            "score": 3368,
            "attributes": {
                "spirit_base": 420
            }
        },
        "昆仑玄石·戒指（破招） 破招等级提升2801点": {
            "id": 16150,
            "name": "昆仑玄石·戒指（破招）",
            "score": 2877,
            "attributes": {
                "surplus_base": 2801
            }
        },
        "昆仑玄石·戒指（内伤） 内功攻击提升849点": {
            "id": 16149,
            "name": "昆仑玄石·戒指（内伤）",
            "score": 2877,
            "attributes": {
                "magical_attack_power_base": 849
            }
        },
        "昆仑玄石·戒指（外伤） 外功攻击提升761点": {
            "id": 16148,
            "name": "昆仑玄石·戒指（外伤）",
            "score": 2877,
            "attributes": {
                "physical_attack_power_base": 761
            }
        },
        "昆仑玄石·戒指（身法） 身法提升359点": {
            "id": 16147,
            "name": "昆仑玄石·戒指（身法）",
            "score": 2877,
            "attributes": {
                "agility_base": 359
            }
        },
        "昆仑玄石·戒指（元气） 元气提升359点": {
            "id": 16146,
            "name": "昆仑玄石·戒指（元气）",
            "score": 2877,
            "attributes": {
                "spunk_base": 359
            }
        },
        "昆仑玄石·戒指（力道） 力道提升359点": {
            "id": 16145,
            "name": "昆仑玄石·戒指（力道）",
            "score": 2877,
            "attributes": {
                "strength_base": 359
            }
        },
        "昆仑玄石·戒指（根骨） 根骨提升359点": {
            "id": 16144,
            "name": "昆仑玄石·戒指（根骨）",
            "score": 2877,
            "attributes": {
                "spirit_base": 359
            }
        }
    },
    "pendant": {
        "暗影·荆岫·腰坠（化劲） 化劲提升2869点": {
            "id": 16407,
            "name": "暗影·荆岫·腰坠（化劲）",
            "score": 5052,
            "attributes": {
                "decritical_power_base": 2869
            }
        },
        "暗影·荆岫·腰坠（体质） 体质提升2126点": {
            "id": 16406,
            "name": "暗影·荆岫·腰坠（体质）",
            "score": 5052,
            "attributes": {
                "vitality_base": 2126
            }
        },
        "暗影·白虹·腰坠（会心） 全会心提升5739点": {
            "id": 16390,
            "name": "暗影·白虹·腰坠（会心）",
            "score": 5052,
            "attributes": {
                "all_critical_strike_base": 5739
            }
        },
        "暗影·白虹·腰坠（体质） 体质提升2126点": {
            "id": 16389,
            "name": "暗影·白虹·腰坠（体质）",
            "score": 5052,
            "attributes": {
                "vitality_base": 2126
            }
        },
        "山海·荆岫·腰坠（化劲） 化劲提升2451点": {
            "id": 16196,
            "name": "山海·荆岫·腰坠（化劲）",
            "score": 4316,
            "attributes": {
                "decritical_power_base": 2451
            }
        },
        "山海·荆岫·腰坠（体质） 体质提升1816点": {
            "id": 16195,
            "name": "山海·荆岫·腰坠（体质）",
            "score": 4316,
            "attributes": {
                "vitality_base": 1816
            }
        },
        "山海·白虹·腰坠（会心） 全会心提升4902点": {
            "id": 16179,
            "name": "山海·白虹·腰坠（会心）",
            "score": 4316,
            "attributes": {
                "all_critical_strike_base": 4902
            }
        },
        "山海·白虹·腰坠（体质） 体质提升1816点": {
            "id": 16178,
            "name": "山海·白虹·腰坠（体质）",
            "score": 4316,
            "attributes": {
                "vitality_base": 1816
            }
        },
        "阆风仙玉·腰坠（化劲） 化劲提升2050点": {
            "id": 16424,
            "name": "阆风仙玉·腰坠（化劲）",
            "score": 3907,
            "attributes": {
                "decritical_power_base": 2050
            }
        },
        "阆风仙玉·腰坠（体质） 体质提升1518点": {
            "id": 16423,
            "name": "阆风仙玉·腰坠（体质）",
            "score": 3907,
            "attributes": {
                "vitality_base": 1518
            }
        },
        "昆仑玄石·腰坠（化劲） 化劲提升1640点": {
            "id": 16369,
            "name": "昆仑玄石·腰坠（化劲）",
            "score": 3368,
            "attributes": {
                "decritical_power_base": 1640
            }
        },
        "昆仑玄石·腰坠（体质） 体质提升1215点": {
            "id": 16368,
            "name": "昆仑玄石·腰坠（体质）",
            "score": 3368,
            "attributes": {
                "vitality_base": 1215
            }
        },
        "昆仑玄石·腰坠（化劲） 化劲提升1401点": {
            "id": 16143,
            "name": "昆仑玄石·腰坠（化劲）",
            "score": 2877,
            "attributes": {
                "decritical_power_base": 1401
            }
        },
        "昆仑玄石·腰坠（体质） 体质提升1037点": {
            "id": 16142,
            "name": "昆仑玄石·腰坠（体质）",
            "score": 2877,
            "attributes": {
                "vitality_base": 1037
            }
        }
    },
    "necklace": {
        "暗影·荆岫·项链（御劲） 御劲提升2869点": {
            "id": 16405,
            "name": "暗影·荆岫·项链（御劲）",
            "score": 5052,
            "attributes": {
                "toughness_base": 2869
            }
        },
        "暗影·荆岫·项链（体质） 体质提升2126点": {
            "id": 16404,
            "name": "暗影·荆岫·项链（体质）",
            "score": 5052,
            "attributes": {
                "vitality_base": 2126
            }
        },
        "暗影·白虹·项链（身法） 身法提升736点": {
            "id": 16388,
            "name": "暗影·白虹·项链（身法）",
            "score": 5052,
            "attributes": {
                "agility_base": 736
            }
        },
        "暗影·白虹·项链（元气） 元气提升736点": {
            "id": 16387,
            "name": "暗影·白虹·项链（元气）",
            "score": 5052,
            "attributes": {
                "spunk_base": 736
            }
        },
        "暗影·白虹·项链（力道） 力道提升736点": {
            "id": 16386,
            "name": "暗影·白虹·项链（力道）",
            "score": 5052,
            "attributes": {
                "strength_base": 736
            }
        },
        "暗影·白虹·项链（根骨） 根骨提升736点": {
            "id": 16385,
            "name": "暗影·白虹·项链（根骨）",
            "score": 5052,
            "attributes": {
                "spirit_base": 736
            }
        },
        "暗影·白虹·项链（体质） 体质提升2126点": {
            "id": 16384,
            "name": "暗影·白虹·项链（体质）",
            "score": 5052,
            "attributes": {
                "vitality_base": 2126
            }
        },
        "山海·荆岫·项链（御劲） 御劲提升2451点": {
            "id": 16194,
            "name": "山海·荆岫·项链（御劲）",
            "score": 4316,
            "attributes": {
                "toughness_base": 2451
            }
        },
        "山海·荆岫·项链（体质） 体质提升1816点": {
            "id": 16193,
            "name": "山海·荆岫·项链（体质）",
            "score": 4316,
            "attributes": {
                "vitality_base": 1816
            }
        },
        "山海·白虹·项链（身法） 身法提升628点": {
            "id": 16177,
            "name": "山海·白虹·项链（身法）",
            "score": 4316,
            "attributes": {
                "agility_base": 628
            }
        },
        "山海·白虹·项链（元气） 元气提升628点": {
            "id": 16176,
            "name": "山海·白虹·项链（元气）",
            "score": 4316,
            "attributes": {
                "spunk_base": 628
            }
        },
        "山海·白虹·项链（力道） 力道提升628点": {
            "id": 16175,
            "name": "山海·白虹·项链（力道）",
            "score": 4316,
            "attributes": {
                "strength_base": 628
            }
        },
        "山海·白虹·项链（根骨） 根骨提升628点": {
            "id": 16174,
            "name": "山海·白虹·项链（根骨）",
            "score": 4316,
            "attributes": {
                "spirit_base": 628
            }
        },
        "山海·白虹·项链（体质） 体质提升1816点": {
            "id": 16173,
            "name": "山海·白虹·项链（体质）",
            "score": 4316,
            "attributes": {
                "vitality_base": 1816
            }
        },
        "阆风仙玉·项链（御劲） 御劲提升2050点": {
            "id": 16422,
            "name": "阆风仙玉·项链（御劲）",
            "score": 3907,
            "attributes": {
                "toughness_base": 2050
            }
        },
        "阆风仙玉·项链（体质） 体质提升1518点": {
            "id": 16421,
            "name": "阆风仙玉·项链（体质）",
            "score": 3907,
            "attributes": {
                "vitality_base": 1518
            }
        },
        "昆仑玄石·项链（御劲） 御劲提升1640点": {
            "id": 16367,
            "name": "昆仑玄石·项链（御劲）",
            "score": 3368,
            "attributes": {
                "toughness_base": 1640
            }
        },
        "昆仑玄石·项链（体质） 体质提升1215点": {
            "id": 16366,
            "name": "昆仑玄石·项链（体质）",
            "score": 3368,
            "attributes": {
                "vitality_base": 1215
            }
        },
        "昆仑玄石·项链（御劲） 御劲提升1401点": {
            "id": 16141,
            "name": "昆仑玄石·项链（御劲）",
            "score": 2877,
            "attributes": {
                "toughness_base": 1401
            }
        },
        "昆仑玄石·项链（体质） 体质提升1037点": {
            "id": 16140,
            "name": "昆仑玄石·项链（体质）",
            "score": 2877,
            "attributes": {
                "vitality_base": 1037
            }
        }
    },
    "jacket": {
        "风语·衣·绣（御劲） 御劲提升1640点": {
            "id": 16316,
            "name": "风语·衣·绣（御劲）",
            "score": 3368,
            "attributes": {
                "toughness_base": 1640
            }
        },
        "风语·衣·绣（化劲） 化劲提升1640点": {
            "id": 16315,
            "name": "风语·衣·绣（化劲）",
            "score": 3368,
            "attributes": {
                "decritical_power_base": 1640
            }
        },
        "风语·衣·绣（招架） 招架提升1640点": {
            "id": 16314,
            "name": "风语·衣·绣（招架）",
            "score": 3368,
            "attributes": {
                "parry_base": 1640
            }
        },
        "风语·衣·绣（内防） 内功防御提升1640点": {
            "id": 16313,
            "name": "风语·衣·绣（内防）",
            "score": 3368,
            "attributes": {
                "magical_shield_base": 1640
            }
        },
        "风语·衣·绣（外防） 外功防御提升1640点": {
            "id": 16312,
            "name": "风语·衣·绣（外防）",
            "score": 3368,
            "attributes": {
                "physical_shield_add": 1640
            }
        },
        "风语·衣·绣（闪避） 闪避提升1640点": {
            "id": 16311,
            "name": "风语·衣·绣（闪避）",
            "score": 3368,
            "attributes": {
                "dodge_base": 1640
            }
        },
        "风语·衣·绣（格挡） 拆招提升1640点": {
            "id": 16310,
            "name": "风语·衣·绣（格挡）",
            "score": 3368,
            "attributes": {
                "parry_value_base": 1640
            }
        },
        "风语·衣·绣（气血） 最大气血值提升16397点": {
            "id": 16309,
            "name": "风语·衣·绣（气血）",
            "score": 3368,
            "attributes": {
                "max_life_base": 16397
            }
        },
        "风语·衣·绣（体质） 体质提升1215点": {
            "id": 16308,
            "name": "风语·衣·绣（体质）",
            "score": 3368,
            "attributes": {
                "vitality_base": 1215
            }
        },
        "风语·衣·绣（御劲） 御劲提升1401点": {
            "id": 16090,
            "name": "风语·衣·绣（御劲）",
            "score": 2877,
            "attributes": {
                "toughness_base": 1401
            }
        },
        "风语·衣·绣（化劲） 化劲提升1401点": {
            "id": 16089,
            "name": "风语·衣·绣（化劲）",
            "score": 2877,
            "attributes": {
                "decritical_power_base": 1401
            }
        },
        "风语·衣·绣（招架） 招架提升1401点": {
            "id": 16088,
            "name": "风语·衣·绣（招架）",
            "score": 2877,
            "attributes": {
                "parry_base": 1401
            }
        },
        "风语·衣·绣（内防） 内功防御提升1401点": {
            "id": 16087,
            "name": "风语·衣·绣（内防）",
            "score": 2877,
            "attributes": {
                "magical_shield_base": 1401
            }
        },
        "风语·衣·绣（外防） 外功防御提升1401点": {
            "id": 16086,
            "name": "风语·衣·绣（外防）",
            "score": 2877,
            "attributes": {
                "physical_shield_add": 1401
            }
        },
        "风语·衣·绣（闪避） 闪避提升1401点": {
            "id": 16085,
            "name": "风语·衣·绣（闪避）",
            "score": 2877,
            "attributes": {
                "dodge_base": 1401
            }
        },
        "风语·衣·绣（格挡） 拆招提升1401点": {
            "id": 16084,
            "name": "风语·衣·绣（格挡）",
            "score": 2877,
            "attributes": {
                "parry_value_base": 1401
            }
        },
        "风语·衣·绣（气血） 最大气血值提升14006点": {
            "id": 16083,
            "name": "风语·衣·绣（气血）",
            "score": 2877,
            "attributes": {
                "max_life_base": 14006
            }
        },
        "风语·衣·绣（体质） 体质提升1037点": {
            "id": 16082,
            "name": "风语·衣·绣（体质）",
            "score": 2877,
            "attributes": {
                "vitality_base": 1037
            }
        },
        "风语·衣·染（御劲） 御劲提升820点": {
            "id": 16339,
            "name": "风语·衣·染（御劲）",
            "score": 1681,
            "attributes": {
                "toughness_base": 820
            }
        },
        "风语·衣·染（化劲） 化劲提升820点": {
            "id": 16338,
            "name": "风语·衣·染（化劲）",
            "score": 1681,
            "attributes": {
                "decritical_power_base": 820
            }
        },
        "风语·衣·染（招架） 招架提升820点": {
            "id": 16337,
            "name": "风语·衣·染（招架）",
            "score": 1681,
            "attributes": {
                "parry_base": 820
            }
        },
        "风语·衣·染（内防） 内功防御提升820点": {
            "id": 16336,
            "name": "风语·衣·染（内防）",
            "score": 1681,
            "attributes": {
                "magical_shield_base": 820
            }
        },
        "风语·衣·染（外防） 外功防御提升820点": {
            "id": 16335,
            "name": "风语·衣·染（外防）",
            "score": 1681,
            "attributes": {
                "physical_shield_add": 820
            }
        },
        "风语·衣·染（闪避） 闪避提升820点": {
            "id": 16334,
            "name": "风语·衣·染（闪避）",
            "score": 1681,
            "attributes": {
                "dodge_base": 820
            }
        },
        "风语·衣·染（格挡） 拆招提升820点": {
            "id": 16333,
            "name": "风语·衣·染（格挡）",
            "score": 1681,
            "attributes": {
                "parry_value_base": 820
            }
        },
        "风语·衣·染（气血） 最大气血值提升8198点": {
            "id": 16332,
            "name": "风语·衣·染（气血）",
            "score": 1681,
            "attributes": {
                "max_life_base": 8199
            }
        },
        "风语·衣·染（体质） 体质提升608点": {
            "id": 16331,
            "name": "风语·衣·染（体质）",
            "score": 1681,
            "attributes": {
                "vitality_base": 608
            }
        },
        "风语·衣·染（御劲） 御劲提升700点": {
            "id": 16113,
            "name": "风语·衣·染（御劲）",
            "score": 1436,
            "attributes": {
                "toughness_base": 700
            }
        },
        "风语·衣·染（化劲） 化劲提升700点": {
            "id": 16112,
            "name": "风语·衣·染（化劲）",
            "score": 1436,
            "attributes": {
                "decritical_power_base": 700
            }
        },
        "风语·衣·染（招架） 招架提升700点": {
            "id": 16111,
            "name": "风语·衣·染（招架）",
            "score": 1436,
            "attributes": {
                "parry_base": 700
            }
        },
        "风语·衣·染（内防） 内功防御提升700点": {
            "id": 16110,
            "name": "风语·衣·染（内防）",
            "score": 1436,
            "attributes": {
                "magical_shield_base": 700
            }
        },
        "风语·衣·染（外防） 外功防御提升700点": {
            "id": 16109,
            "name": "风语·衣·染（外防）",
            "score": 1436,
            "attributes": {
                "physical_shield_add": 700
            }
        },
        "风语·衣·染（闪避） 闪避提升700点": {
            "id": 16108,
            "name": "风语·衣·染（闪避）",
            "score": 1436,
            "attributes": {
                "dodge_base": 700
            }
        },
        "风语·衣·染（格挡） 拆招提升700点": {
            "id": 16107,
            "name": "风语·衣·染（格挡）",
            "score": 1436,
            "attributes": {
                "parry_value_base": 700
            }
        },
        "风语·衣·染（气血） 最大气血值提升7003点": {
            "id": 16106,
            "name": "风语·衣·染（气血）",
            "score": 1436,
            "attributes": {
                "max_life_base": 7003
            }
        },
        "风语·衣·染（体质） 体质提升519点": {
            "id": 16105,
            "name": "风语·衣·染（体质）",
            "score": 1436,
            "attributes": {
                "vitality_base": 519
            }
        },
        "远风·衣·无双 上衣无双等级提高978": {
            "id": 16364,
            "name": "远风·衣·无双",
            "score": 1341,
            "attributes": {
                "strain_base": 978
            }
        },
        "远风·衣·内防 上衣内功防御等级提高656": {
            "id": 16355,
            "name": "远风·衣·内防",
            "score": 1341,
            "attributes": {
                "magical_shield_base": 656
            }
        },
        "远风·衣·外防 上衣外功防御等级提高656": {
            "id": 16354,
            "name": "远风·衣·外防",
            "score": 1341,
            "attributes": {
                "physical_shield_base": 656
            }
        },
        "远风·衣·御劲 上衣御劲等级提高656": {
            "id": 16353,
            "name": "远风·衣·御劲",
            "score": 1341,
            "attributes": {
                "toughness_base": 656
            }
        },
        "远风·衣·化劲 上衣化劲等级提高656": {
            "id": 16352,
            "name": "远风·衣·化劲",
            "score": 1341,
            "attributes": {
                "decritical_power_base": 656
            }
        },
        "远风·衣·无双 上衣无双等级提高836": {
            "id": 16138,
            "name": "远风·衣·无双",
            "score": 1146,
            "attributes": {
                "strain_base": 836
            }
        },
        "远风·衣·内防 上衣内功防御等级提高560": {
            "id": 16129,
            "name": "远风·衣·内防",
            "score": 1146,
            "attributes": {
                "magical_shield_base": 560
            }
        },
        "远风·衣·外防 上衣外功防御等级提高560": {
            "id": 16128,
            "name": "远风·衣·外防",
            "score": 1146,
            "attributes": {
                "physical_shield_base": 560
            }
        },
        "远风·衣·御劲 上衣御劲等级提高560": {
            "id": 16127,
            "name": "远风·衣·御劲",
            "score": 1146,
            "attributes": {
                "toughness_base": 560
            }
        },
        "远风·衣·化劲 上衣化劲等级提高560": {
            "id": 16126,
            "name": "远风·衣·化劲",
            "score": 1146,
            "attributes": {
                "decritical_power_base": 560
            }
        }
    },
    "shoes": {
        "风语·鞋·绣（急速） 加速提升3279点": {
            "id": 16307,
            "name": "风语·鞋·绣（急速）",
            "score": 3368,
            "attributes": {
                "haste_base": 3279
            }
        },
        "风语·鞋·绣（破招） 破招提升3279点": {
            "id": 16306,
            "name": "风语·鞋·绣（破招）",
            "score": 3368,
            "attributes": {
                "surplus_base": 3279
            }
        },
        "风语·鞋·绣（治疗） 疗伤成效提升1592点": {
            "id": 16305,
            "name": "风语·鞋·绣（治疗）",
            "score": 3368,
            "attributes": {
                "therapy_power_base": 1592
            }
        },
        "风语·鞋·绣（内攻） 内功攻击提升994点": {
            "id": 16304,
            "name": "风语·鞋·绣（内攻）",
            "score": 3368,
            "attributes": {
                "magical_attack_power_base": 994
            }
        },
        "风语·鞋·绣（外攻） 外功攻击提升891点": {
            "id": 16303,
            "name": "风语·鞋·绣（外攻）",
            "score": 3368,
            "attributes": {
                "physical_attack_power_base": 891
            }
        },
        "风语·鞋·绣（急速） 加速提升2801点": {
            "id": 16081,
            "name": "风语·鞋·绣（急速）",
            "score": 2877,
            "attributes": {
                "haste_base": 2801
            }
        },
        "风语·鞋·绣（破招） 破招提升2801点": {
            "id": 16080,
            "name": "风语·鞋·绣（破招）",
            "score": 2877,
            "attributes": {
                "surplus_base": 2801
            }
        },
        "风语·鞋·绣（治疗） 疗伤成效提升1360点": {
            "id": 16079,
            "name": "风语·鞋·绣（治疗）",
            "score": 2877,
            "attributes": {
                "therapy_power_base": 1360
            }
        },
        "风语·鞋·绣（内攻） 内功攻击提升849点": {
            "id": 16078,
            "name": "风语·鞋·绣（内攻）",
            "score": 2877,
            "attributes": {
                "magical_attack_power_base": 849
            }
        },
        "风语·鞋·绣（外攻） 外功攻击提升761点": {
            "id": 16077,
            "name": "风语·鞋·绣（外攻）",
            "score": 2877,
            "attributes": {
                "physical_attack_power_base": 761
            }
        },
        "风语·鞋·染（急速） 加速提升1640点": {
            "id": 16330,
            "name": "风语·鞋·染（急速）",
            "score": 1681,
            "attributes": {
                "haste_base": 1640
            }
        },
        "风语·鞋·染（破招） 破招提升1640点": {
            "id": 16329,
            "name": "风语·鞋·染（破招）",
            "score": 1681,
            "attributes": {
                "surplus_base": 1640
            }
        },
        "风语·鞋·染（治疗） 疗伤成效提升796点": {
            "id": 16328,
            "name": "风语·鞋·染（治疗）",
            "score": 1681,
            "attributes": {
                "therapy_power_base": 796
            }
        },
        "风语·鞋·染（内攻） 内功攻击提升497点": {
            "id": 16327,
            "name": "风语·鞋·染（内攻）",
            "score": 1681,
            "attributes": {
                "magical_attack_power_base": 497
            }
        },
        "风语·鞋·染（外攻） 外功攻击提升446点": {
            "id": 16326,
            "name": "风语·鞋·染（外攻）",
            "score": 1681,
            "attributes": {
                "physical_attack_power_base": 446
            }
        },
        "风语·鞋·染（急速） 加速提升1401点": {
            "id": 16104,
            "name": "风语·鞋·染（急速）",
            "score": 1436,
            "attributes": {
                "haste_base": 1401
            }
        },
        "风语·鞋·染（破招） 破招提升1401点": {
            "id": 16103,
            "name": "风语·鞋·染（破招）",
            "score": 1436,
            "attributes": {
                "surplus_base": 1401
            }
        },
        "风语·鞋·染（治疗） 疗伤成效提升680点": {
            "id": 16102,
            "name": "风语·鞋·染（治疗）",
            "score": 1436,
            "attributes": {
                "therapy_power_base": 680
            }
        },
        "风语·鞋·染（内攻） 内功攻击提升424点": {
            "id": 16101,
            "name": "风语·鞋·染（内攻）",
            "score": 1436,
            "attributes": {
                "magical_attack_power_base": 424
            }
        },
        "风语·鞋·染（外攻） 外功攻击提升381点": {
            "id": 16100,
            "name": "风语·鞋·染（外攻）",
            "score": 1436,
            "attributes": {
                "physical_attack_power_base": 381
            }
        },
        "远风·鞋·内攻 鞋子内功攻击提高397": {
            "id": 16363,
            "name": "远风·鞋·内攻",
            "score": 1341,
            "attributes": {
                "magical_attack_power_base": 397
            }
        },
        "远风·鞋·外攻 鞋子外功攻击提高356": {
            "id": 16362,
            "name": "远风·鞋·外攻",
            "score": 1341,
            "attributes": {
                "physical_attack_power_base": 356
            }
        },
        "远风·鞋·破招 鞋子破招等级提高1312": {
            "id": 16361,
            "name": "远风·鞋·破招",
            "score": 1341,
            "attributes": {
                "surplus_base": 1312
            }
        },
        "远风·鞋·急速 鞋子加速等级提高1312": {
            "id": 16360,
            "name": "远风·鞋·急速",
            "score": 1341,
            "attributes": {
                "haste_base": 1312
            }
        },
        "远风·鞋·内攻 鞋子内功攻击提高340": {
            "id": 16137,
            "name": "远风·鞋·内攻",
            "score": 1146,
            "attributes": {
                "magical_attack_power_base": 340
            }
        },
        "远风·鞋·外攻 鞋子外功攻击提高304": {
            "id": 16136,
            "name": "远风·鞋·外攻",
            "score": 1146,
            "attributes": {
                "physical_attack_power_base": 304
            }
        },
        "远风·鞋·破招 鞋子破招等级提高1120": {
            "id": 16135,
            "name": "远风·鞋·破招",
            "score": 1146,
            "attributes": {
                "surplus_base": 1120
            }
        },
        "远风·鞋·急速 鞋子加速等级提高1120": {
            "id": 16134,
            "name": "远风·鞋·急速",
            "score": 1146,
            "attributes": {
                "haste_base": 1120
            }
        }
    },
    "wrist": {
        "风语·腕·绣（无双） 无双等级提升3279点": {
            "id": 16302,
            "name": "风语·腕·绣（无双）",
            "score": 3368,
            "attributes": {
                "strain_base": 3279
            }
        },
        "风语·腕·绣（会心） 全会心提升3279点": {
            "id": 16301,
            "name": "风语·腕·绣（会心）",
            "score": 3368,
            "attributes": {
                "all_critical_strike_base": 3279
            }
        },
        "风语·腕·绣（内破） 内功破防等级提升3279点": {
            "id": 16300,
            "name": "风语·腕·绣（内破）",
            "score": 3368,
            "attributes": {
                "magical_overcome_base": 3279
            }
        },
        "风语·腕·绣（外破） 外功破防等级提升3279点": {
            "id": 16299,
            "name": "风语·腕·绣（外破）",
            "score": 3368,
            "attributes": {
                "physical_overcome_base": 3279
            }
        },
        "风语·腕·绣（根骨） 根骨提升420点": {
            "id": 16297,
            "name": "风语·腕·绣（根骨）",
            "score": 3368,
            "attributes": {
                "spirit_base": 420
            }
        },
        "风语·腕·绣（力道） 力道提升420点": {
            "id": 16296,
            "name": "风语·腕·绣（力道）",
            "score": 3368,
            "attributes": {
                "strength_base": 420
            }
        },
        "风语·腕·绣（元气） 元气提升420点": {
            "id": 16295,
            "name": "风语·腕·绣（元气）",
            "score": 3368,
            "attributes": {
                "spunk_base": 420
            }
        },
        "风语·腕·绣（身法） 身法提升420点": {
            "id": 16294,
            "name": "风语·腕·绣（身法）",
            "score": 3368,
            "attributes": {
                "agility_base": 420
            }
        },
        "风语·腕·绣（无双） 无双等级提升2801点": {
            "id": 16076,
            "name": "风语·腕·绣（无双）",
            "score": 2877,
            "attributes": {
                "strain_base": 2801
            }
        },
        "风语·腕·绣（会心） 全会心提升2801点": {
            "id": 16075,
            "name": "风语·腕·绣（会心）",
            "score": 2877,
            "attributes": {
                "all_critical_strike_base": 2801
            }
        },
        "风语·腕·绣（内破） 内功破防等级提升2801点": {
            "id": 16074,
            "name": "风语·腕·绣（内破）",
            "score": 2877,
            "attributes": {
                "magical_overcome_base": 2801
            }
        },
        "风语·腕·绣（外破） 外功破防等级提升2801点": {
            "id": 16073,
            "name": "风语·腕·绣（外破）",
            "score": 2877,
            "attributes": {
                "physical_overcome_base": 2801
            }
        },
        "风语·腕·绣（根骨） 根骨提升359点": {
            "id": 16071,
            "name": "风语·腕·绣（根骨）",
            "score": 2877,
            "attributes": {
                "spirit_base": 359
            }
        },
        "风语·腕·绣（力道） 力道提升359点": {
            "id": 16070,
            "name": "风语·腕·绣（力道）",
            "score": 2877,
            "attributes": {
                "strength_base": 359
            }
        },
        "风语·腕·绣（元气） 元气提升359点": {
            "id": 16069,
            "name": "风语·腕·绣（元气）",
            "score": 2877,
            "attributes": {
                "spunk_base": 359
            }
        },
        "风语·腕·绣（身法） 身法提升359点": {
            "id": 16068,
            "name": "风语·腕·绣（身法）",
            "score": 2877,
            "attributes": {
                "agility_base": 359
            }
        },
        "风语·腕·染（无双） 无双等级提升1640点": {
            "id": 16325,
            "name": "风语·腕·染（无双）",
            "score": 1681,
            "attributes": {
                "strain_base": 1640
            }
        },
        "风语·腕·染（会心） 全会心提升1640点": {
            "id": 16324,
            "name": "风语·腕·染（会心）",
            "score": 1681,
            "attributes": {
                "all_critical_strike_base": 1640
            }
        },
        "风语·腕·染（内破） 内功破防等级提升1640点": {
            "id": 16323,
            "name": "风语·腕·染（内破）",
            "score": 1681,
            "attributes": {
                "magical_overcome_base": 1640
            }
        },
        "风语·腕·染（外破） 外功破防等级提升1640点": {
            "id": 16322,
            "name": "风语·腕·染（外破）",
            "score": 1681,
            "attributes": {
                "physical_overcome_base": 1640
            }
        },
        "风语·腕·染（根骨） 根骨提升210点": {
            "id": 16320,
            "name": "风语·腕·染（根骨）",
            "score": 1681,
            "attributes": {
                "spirit_base": 210
            }
        },
        "风语·腕·染（力道） 力道提升210点": {
            "id": 16319,
            "name": "风语·腕·染（力道）",
            "score": 1681,
            "attributes": {
                "strength_base": 210
            }
        },
        "风语·腕·染（元气） 元气提升210点": {
            "id": 16318,
            "name": "风语·腕·染（元气）",
            "score": 1681,
            "attributes": {
                "spunk_base": 210
            }
        },
        "风语·腕·染（身法） 身法提升210点": {
            "id": 16317,
            "name": "风语·腕·染（身法）",
            "score": 1681,
            "attributes": {
                "agility_base": 210
            }
        },
        "风语·腕·染（无双） 无双等级提升1401点": {
            "id": 16099,
            "name": "风语·腕·染（无双）",
            "score": 1436,
            "attributes": {
                "strain_base": 1401
            }
        },
        "风语·腕·染（会心） 全会心提升1401点": {
            "id": 16098,
            "name": "风语·腕·染（会心）",
            "score": 1436,
            "attributes": {
                "all_critical_strike_base": 1401
            }
        },
        "风语·腕·染（内破） 内功破防等级提升1401点": {
            "id": 16097,
            "name": "风语·腕·染（内破）",
            "score": 1436,
            "attributes": {
                "magical_overcome_base": 1401
            }
        },
        "风语·腕·染（外破） 外功破防等级提升1401点": {
            "id": 16096,
            "name": "风语·腕·染（外破）",
            "score": 1436,
            "attributes": {
                "physical_overcome_base": 1401
            }
        },
        "风语·腕·染（根骨） 根骨提升180点": {
            "id": 16094,
            "name": "风语·腕·染（根骨）",
            "score": 1436,
            "attributes": {
                "spirit_base": 180
            }
        },
        "风语·腕·染（力道） 力道提升180点": {
            "id": 16093,
            "name": "风语·腕·染（力道）",
            "score": 1436,
            "attributes": {
                "strength_base": 180
            }
        },
        "风语·腕·染（元气） 元气提升180点": {
            "id": 16092,
            "name": "风语·腕·染（元气）",
            "score": 1436,
            "attributes": {
                "spunk_base": 180
            }
        },
        "风语·腕·染（身法） 身法提升180点": {
            "id": 16091,
            "name": "风语·腕·染（身法）",
            "score": 1436,
            "attributes": {
                "agility_base": 180
            }
        },
        "远风·腕·无双 护腕无双等级提高1312": {
            "id": 16351,
            "name": "远风·腕·无双",
            "score": 1341,
            "attributes": {
                "strain_base": 1312
            }
        },
        "远风·腕·内破 护腕内功破防等级提高1312": {
            "id": 16350,
            "name": "远风·腕·内破",
            "score": 1341,
            "attributes": {
                "magical_overcome_base": 1312
            }
        },
        "远风·腕·外破 护腕外功破防等级提高1312": {
            "id": 16349,
            "name": "远风·腕·外破",
            "score": 1341,
            "attributes": {
                "physical_overcome_base": 1312
            }
        },
        "远风·腕·会心 护腕全会心等级提高1312": {
            "id": 16341,
            "name": "远风·腕·会心",
            "score": 1341,
            "attributes": {
                "all_critical_strike_base": 1312
            }
        },
        "远风·腕·无双 护腕无双等级提高1120": {
            "id": 16125,
            "name": "远风·腕·无双",
            "score": 1146,
            "attributes": {
                "strain_base": 1120
            }
        },
        "远风·腕·内破 护腕内功破防等级提高1120": {
            "id": 16124,
            "name": "远风·腕·内破",
            "score": 1146,
            "attributes": {
                "magical_overcome_base": 1120
            }
        },
        "远风·腕·外破 护腕外功破防等级提高1120": {
            "id": 16123,
            "name": "远风·腕·外破",
            "score": 1146,
            "attributes": {
                "physical_overcome_base": 1120
            }
        },
        "远风·腕·会心 护腕全会心等级提高1120": {
            "id": 16115,
            "name": "远风·腕·会心",
            "score": 1146,
            "attributes": {
                "all_critical_strike_base": 1120
            }
        }
    },
    "belt": {
        "风语·腰·铸（化劲） 化劲提升1640点": {
            "id": 16261,
            "name": "风语·腰·铸（化劲）",
            "score": 3368,
            "attributes": {
                "decritical_power_base": 1640
            }
        },
        "风语·腰·铸（御劲） 御劲提升1640点": {
            "id": 16260,
            "name": "风语·腰·铸（御劲）",
            "score": 3368,
            "attributes": {
                "toughness_base": 1640
            }
        },
        "风语·腰·铸（招架） 招架提升1640点": {
            "id": 16259,
            "name": "风语·腰·铸（招架）",
            "score": 3368,
            "attributes": {
                "parry_base": 1640
            }
        },
        "风语·腰·铸（内防） 内功防御提升1640点": {
            "id": 16258,
            "name": "风语·腰·铸（内防）",
            "score": 3368,
            "attributes": {
                "magical_shield_base": 1640
            }
        },
        "风语·腰·铸（外防） 外功防御提升1640点": {
            "id": 16257,
            "name": "风语·腰·铸（外防）",
            "score": 3368,
            "attributes": {
                "physical_shield_add": 1640
            }
        },
        "风语·腰·铸（闪避） 闪避提升1640点": {
            "id": 16256,
            "name": "风语·腰·铸（闪避）",
            "score": 3368,
            "attributes": {
                "dodge_base": 1640
            }
        },
        "风语·腰·铸（格挡） 拆招提升1640点": {
            "id": 16255,
            "name": "风语·腰·铸（格挡）",
            "score": 3368,
            "attributes": {
                "parry_value_base": 1640
            }
        },
        "风语·腰·铸（气血） 最大气血值提升16397点": {
            "id": 16254,
            "name": "风语·腰·铸（气血）",
            "score": 3368,
            "attributes": {
                "max_life_base": 16397
            }
        },
        "风语·腰·铸（体质） 体质提升1215点": {
            "id": 16253,
            "name": "风语·腰·铸（体质）",
            "score": 3368,
            "attributes": {
                "vitality_base": 1215
            }
        },
        "风语·腰·铸（化劲） 化劲提升1401点": {
            "id": 16035,
            "name": "风语·腰·铸（化劲）",
            "score": 2877,
            "attributes": {
                "decritical_power_base": 1401
            }
        },
        "风语·腰·铸（御劲） 御劲提升1401点": {
            "id": 16034,
            "name": "风语·腰·铸（御劲）",
            "score": 2877,
            "attributes": {
                "toughness_base": 1401
            }
        },
        "风语·腰·铸（招架） 招架提升1401点": {
            "id": 16033,
            "name": "风语·腰·铸（招架）",
            "score": 2877,
            "attributes": {
                "parry_base": 1401
            }
        },
        "风语·腰·铸（内防） 内功防御提升1401点": {
            "id": 16032,
            "name": "风语·腰·铸（内防）",
            "score": 2877,
            "attributes": {
                "magical_shield_base": 1401
            }
        },
        "风语·腰·铸（外防） 外功防御提升1401点": {
            "id": 16031,
            "name": "风语·腰·铸（外防）",
            "score": 2877,
            "attributes": {
                "physical_shield_add": 1401
            }
        },
        "风语·腰·铸（闪避） 闪避提升1401点": {
            "id": 16030,
            "name": "风语·腰·铸（闪避）",
            "score": 2877,
            "attributes": {
                "dodge_base": 1401
            }
        },
        "风语·腰·铸（格挡） 拆招提升1401点": {
            "id": 16029,
            "name": "风语·腰·铸（格挡）",
            "score": 2877,
            "attributes": {
                "parry_value_base": 1401
            }
        },
        "风语·腰·铸（气血） 最大气血值提升14006点": {
            "id": 16028,
            "name": "风语·腰·铸（气血）",
            "score": 2877,
            "attributes": {
                "max_life_base": 14006
            }
        },
        "风语·腰·铸（体质） 体质提升1037点": {
            "id": 16027,
            "name": "风语·腰·铸（体质）",
            "score": 2877,
            "attributes": {
                "vitality_base": 1037
            }
        },
        "风语·腰·甲（化劲） 化劲提升820点": {
            "id": 16284,
            "name": "风语·腰·甲（化劲）",
            "score": 1681,
            "attributes": {
                "toughness_base": 820
            }
        },
        "风语·腰·甲（御劲） 御劲提升820点": {
            "id": 16283,
            "name": "风语·腰·甲（御劲）",
            "score": 1681,
            "attributes": {
                "decritical_power_base": 820
            }
        },
        "风语·腰·甲（招架） 招架提升820点": {
            "id": 16282,
            "name": "风语·腰·甲（招架）",
            "score": 1681,
            "attributes": {
                "parry_base": 820
            }
        },
        "风语·腰·甲（内防） 内功防御提升820点": {
            "id": 16281,
            "name": "风语·腰·甲（内防）",
            "score": 1681,
            "attributes": {
                "magical_shield_base": 820
            }
        },
        "风语·腰·甲（外防） 外功防御提升820点": {
            "id": 16280,
            "name": "风语·腰·甲（外防）",
            "score": 1681,
            "attributes": {
                "physical_shield_add": 820
            }
        },
        "风语·腰·甲（闪避） 闪避提升820点": {
            "id": 16279,
            "name": "风语·腰·甲（闪避）",
            "score": 1681,
            "attributes": {
                "dodge_base": 820
            }
        },
        "风语·腰·甲（格挡） 拆招提升820点": {
            "id": 16278,
            "name": "风语·腰·甲（格挡）",
            "score": 1681,
            "attributes": {
                "parry_value_base": 820
            }
        },
        "风语·腰·甲（气血） 最大气血值提升8199点": {
            "id": 16277,
            "name": "风语·腰·甲（气血）",
            "score": 1681,
            "attributes": {
                "max_life_base": 8199
            }
        },
        "风语·腰·甲（体质） 体质提升608点": {
            "id": 16276,
            "name": "风语·腰·甲（体质）",
            "score": 1681,
            "attributes": {
                "vitality_base": 608
            }
        },
        "风语·腰·甲（化劲） 化劲提升700点": {
            "id": 16058,
            "name": "风语·腰·甲（化劲）",
            "score": 1436,
            "attributes": {
                "toughness_base": 700
            }
        },
        "风语·腰·甲（御劲） 御劲提升700点": {
            "id": 16057,
            "name": "风语·腰·甲（御劲）",
            "score": 1436,
            "attributes": {
                "decritical_power_base": 700
            }
        },
        "风语·腰·甲（招架） 招架提升700点": {
            "id": 16056,
            "name": "风语·腰·甲（招架）",
            "score": 1436,
            "attributes": {
                "parry_base": 700
            }
        },
        "风语·腰·甲（内防） 内功防御提升700点": {
            "id": 16055,
            "name": "风语·腰·甲（内防）",
            "score": 1436,
            "attributes": {
                "magical_shield_base": 700
            }
        },
        "风语·腰·甲（外防） 外功防御提升700点": {
            "id": 16054,
            "name": "风语·腰·甲（外防）",
            "score": 1436,
            "attributes": {
                "physical_shield_add": 700
            }
        },
        "风语·腰·甲（闪避） 闪避提升700点": {
            "id": 16053,
            "name": "风语·腰·甲（闪避）",
            "score": 1436,
            "attributes": {
                "dodge_base": 700
            }
        },
        "风语·腰·甲（格挡） 拆招提升700点": {
            "id": 16052,
            "name": "风语·腰·甲（格挡）",
            "score": 1436,
            "attributes": {
                "parry_value_base": 700
            }
        },
        "风语·腰·甲（气血） 最大气血值提升7003点": {
            "id": 16051,
            "name": "风语·腰·甲（气血）",
            "score": 1436,
            "attributes": {
                "max_life_base": 7003
            }
        },
        "风语·腰·甲（体质） 体质提升519点": {
            "id": 16050,
            "name": "风语·腰·甲（体质）",
            "score": 1436,
            "attributes": {
                "vitality_base": 519
            }
        },
        "远风·腰·无双 腰带无双等级提高978": {
            "id": 16365,
            "name": "远风·腰·无双",
            "score": 1341,
            "attributes": {
                "strain_base": 978
            }
        },
        "远风·腰·内防 腰带内功防御等级提高656": {
            "id": 16359,
            "name": "远风·腰·内防",
            "score": 1341,
            "attributes": {
                "magical_shield_base": 656
            }
        },
        "远风·腰·外防 腰带外功防御等级提高656": {
            "id": 16358,
            "name": "远风·腰·外防",
            "score": 1341,
            "attributes": {
                "physical_shield_base": 656
            }
        },
        "远风·腰·御劲 腰带御劲等级提高656": {
            "id": 16357,
            "name": "远风·腰·御劲",
            "score": 1341,
            "attributes": {
                "toughness_base": 656
            }
        },
        "远风·腰·化劲 腰带化劲等级提高656": {
            "id": 16356,
            "name": "远风·腰·化劲",
            "score": 1341,
            "attributes": {
                "decritical_power_base": 656
            }
        },
        "远风·腰·无双 腰带无双等级提高836": {
            "id": 16139,
            "name": "远风·腰·无双",
            "score": 1146,
            "attributes": {
                "strain_base": 836
            }
        },
        "远风·腰·内防 腰带内功防御等级提高560": {
            "id": 16133,
            "name": "远风·腰·内防",
            "score": 1146,
            "attributes": {
                "magical_shield_base": 560
            }
        },
        "远风·腰·外防 腰带外功防御等级提高560": {
            "id": 16132,
            "name": "远风·腰·外防",
            "score": 1146,
            "attributes": {
                "physical_shield_base": 560
            }
        },
        "远风·腰·御劲 腰带御劲等级提高560": {
            "id": 16131,
            "name": "远风·腰·御劲",
            "score": 1146,
            "attributes": {
                "toughness_base": 560
            }
        },
        "远风·腰·化劲 腰带化劲等级提高560": {
            "id": 16130,
            "name": "远风·腰·化劲",
            "score": 1146,
            "attributes": {
                "decritical_power_base": 560
            }
        }
    },
    "hat": {
        "风语·头·铸（急速） 加速提升3279点": {
            "id": 16252,
            "name": "风语·头·铸（急速）",
            "score": 3368,
            "attributes": {
                "haste_base": 3279
            }
        },
        "风语·头·铸（破招） 破招提升3279点": {
            "id": 16251,
            "name": "风语·头·铸（破招）",
            "score": 3368,
            "attributes": {
                "surplus_base": 3279
            }
        },
        "风语·头·铸（治疗） 疗伤成效提升1592点": {
            "id": 16250,
            "name": "风语·头·铸（治疗）",
            "score": 3368,
            "attributes": {
                "therapy_power_base": 1592
            }
        },
        "风语·头·铸（内攻） 内功攻击提升994点": {
            "id": 16249,
            "name": "风语·头·铸（内攻）",
            "score": 3368,
            "attributes": {
                "magical_attack_power_base": 994
            }
        },
        "风语·头·铸（外攻） 外功攻击提升891点": {
            "id": 16248,
            "name": "风语·头·铸（外攻）",
            "score": 3368,
            "attributes": {
                "physical_attack_power_base": 891
            }
        },
        "风语·头·铸（急速） 加速提升2801点": {
            "id": 16026,
            "name": "风语·头·铸（急速）",
            "score": 2877,
            "attributes": {
                "haste_base": 2801
            }
        },
        "风语·头·铸（破招） 破招提升2801点": {
            "id": 16025,
            "name": "风语·头·铸（破招）",
            "score": 2877,
            "attributes": {
                "surplus_base": 2801
            }
        },
        "风语·头·铸（治疗） 疗伤成效提升1360点": {
            "id": 16024,
            "name": "风语·头·铸（治疗）",
            "score": 2877,
            "attributes": {
                "therapy_power_base": 1360
            }
        },
        "风语·头·铸（内攻） 内功攻击提升849点": {
            "id": 16023,
            "name": "风语·头·铸（内攻）",
            "score": 2877,
            "attributes": {
                "magical_attack_power_base": 849
            }
        },
        "风语·头·铸（外攻） 外功攻击提升761点": {
            "id": 16022,
            "name": "风语·头·铸（外攻）",
            "score": 2877,
            "attributes": {
                "physical_attack_power_base": 761
            }
        },
        "风语·头·甲（急速） 加速提升1640点": {
            "id": 16275,
            "name": "风语·头·甲（急速）",
            "score": 1681,
            "attributes": {
                "haste_base": 1640
            }
        },
        "风语·头·甲（破招） 破招提升1640点": {
            "id": 16274,
            "name": "风语·头·甲（破招）",
            "score": 1681,
            "attributes": {
                "surplus_base": 1640
            }
        },
        "风语·头·甲（治疗） 疗伤成效提升796点": {
            "id": 16273,
            "name": "风语·头·甲（治疗）",
            "score": 1681,
            "attributes": {
                "therapy_power_base": 796
            }
        },
        "风语·头·甲（内攻） 内功攻击提升497点": {
            "id": 16272,
            "name": "风语·头·甲（内攻）",
            "score": 1681,
            "attributes": {
                "magical_attack_power_base": 497
            }
        },
        "风语·头·甲（外攻） 外功攻击提升446点": {
            "id": 16271,
            "name": "风语·头·甲（外攻）",
            "score": 1681,
            "attributes": {
                "physical_attack_power_base": 446
            }
        },
        "风语·头·甲（急速） 加速提升1401点": {
            "id": 16049,
            "name": "风语·头·甲（急速）",
            "score": 1436,
            "attributes": {
                "haste_base": 1401
            }
        },
        "风语·头·甲（破招） 破招提升1401点": {
            "id": 16048,
            "name": "风语·头·甲（破招）",
            "score": 1436,
            "attributes": {
                "surplus_base": 1401
            }
        },
        "风语·头·甲（治疗） 疗伤成效提升680点": {
            "id": 16047,
            "name": "风语·头·甲（治疗）",
            "score": 1436,
            "attributes": {
                "therapy_power_base": 680
            }
        },
        "风语·头·甲（内攻） 内功攻击提升424点": {
            "id": 16046,
            "name": "风语·头·甲（内攻）",
            "score": 1436,
            "attributes": {
                "magical_attack_power_base": 424
            }
        },
        "风语·头·甲（外攻） 外功攻击提升381点": {
            "id": 16045,
            "name": "风语·头·甲（外攻）",
            "score": 1436,
            "attributes": {
                "physical_attack_power_base": 381
            }
        },
        "远风·头·治疗 帽子治疗成效提高637": {
            "id": 16348,
            "name": "远风·头·治疗",
            "score": 1341,
            "attributes": {
                "therapy_power_base": 637
            }
        },
        "远风·头·内攻 帽子内功攻击提高397": {
            "id": 16347,
            "name": "远风·头·内攻",
            "score": 1341,
            "attributes": {
                "magical_attack_power_base": 397
            }
        },
        "远风·头·外攻 帽子外功攻击提高356": {
            "id": 16346,
            "name": "远风·头·外攻",
            "score": 1341,
            "attributes": {
                "physical_attack_power_base": 356
            }
        },
        "远风·头·急速 帽子加速等级提高1312": {
            "id": 16342,
            "name": "远风·头·急速",
            "score": 1341,
            "attributes": {
                "haste_base": 1312
            }
        },
        "远风·头·治疗 帽子治疗成效提高544": {
            "id": 16122,
            "name": "远风·头·治疗",
            "score": 1146,
            "attributes": {
                "therapy_power_base": 544
            }
        },
        "远风·头·内攻 帽子内功攻击提高340": {
            "id": 16121,
            "name": "远风·头·内攻",
            "score": 1146,
            "attributes": {
                "magical_attack_power_base": 340
            }
        },
        "远风·头·外攻 帽子外功攻击提高304": {
            "id": 16120,
            "name": "远风·头·外攻",
            "score": 1146,
            "attributes": {
                "physical_attack_power_base": 304
            }
        },
        "远风·头·急速 帽子加速等级提高1120": {
            "id": 16116,
            "name": "远风·头·急速",
            "score": 1146,
            "attributes": {
                "haste_base": 1120
            }
        }
    },
    "bottoms": {
        "风语·裤·铸（无双） 无双等级提升3279点": {
            "id": 16247,
            "name": "风语·裤·铸（无双）",
            "score": 3368,
            "attributes": {
                "strain_base": 3279
            }
        },
        "风语·裤·铸（会心） 全会心提升3279点": {
            "id": 16246,
            "name": "风语·裤·铸（会心）",
            "score": 3368,
            "attributes": {
                "all_critical_strike_base": 3279
            }
        },
        "风语·裤·铸（内破） 内功破防等级提升3279点": {
            "id": 16245,
            "name": "风语·裤·铸（内破）",
            "score": 3368,
            "attributes": {
                "magical_overcome_base": 3279
            }
        },
        "风语·裤·铸（外破） 外功破防等级提升3279点": {
            "id": 16244,
            "name": "风语·裤·铸（外破）",
            "score": 3368,
            "attributes": {
                "physical_overcome_base": 3279
            }
        },
        "风语·裤·铸（根骨） 根骨提升420点": {
            "id": 16242,
            "name": "风语·裤·铸（根骨）",
            "score": 3368,
            "attributes": {
                "spirit_base": 420
            }
        },
        "风语·裤·铸（力道） 力道提升420点": {
            "id": 16241,
            "name": "风语·裤·铸（力道）",
            "score": 3368,
            "attributes": {
                "strength_base": 420
            }
        },
        "风语·裤·铸（元气） 元气提升420点": {
            "id": 16240,
            "name": "风语·裤·铸（元气）",
            "score": 3368,
            "attributes": {
                "spunk_base": 420
            }
        },
        "风语·裤·铸（身法） 身法提升420点": {
            "id": 16239,
            "name": "风语·裤·铸（身法）",
            "score": 3368,
            "attributes": {
                "agility_base": 420
            }
        },
        "风语·裤·铸（无双） 无双等级提升2801点": {
            "id": 16021,
            "name": "风语·裤·铸（无双）",
            "score": 2877,
            "attributes": {
                "strain_base": 2801
            }
        },
        "风语·裤·铸（会心） 全会心提升2801点": {
            "id": 16020,
            "name": "风语·裤·铸（会心）",
            "score": 2877,
            "attributes": {
                "all_critical_strike_base": 2801
            }
        },
        "风语·裤·铸（内破） 内功破防等级提升2801点": {
            "id": 16019,
            "name": "风语·裤·铸（内破）",
            "score": 2877,
            "attributes": {
                "magical_overcome_base": 2801
            }
        },
        "风语·裤·铸（外破） 外功破防等级提升2801点": {
            "id": 16018,
            "name": "风语·裤·铸（外破）",
            "score": 2877,
            "attributes": {
                "physical_overcome_base": 2801
            }
        },
        "风语·裤·铸（根骨） 根骨提升359点": {
            "id": 16016,
            "name": "风语·裤·铸（根骨）",
            "score": 2877,
            "attributes": {
                "spirit_base": 359
            }
        },
        "风语·裤·铸（力道） 力道提升359点": {
            "id": 16015,
            "name": "风语·裤·铸（力道）",
            "score": 2877,
            "attributes": {
                "strength_base": 359
            }
        },
        "风语·裤·铸（元气） 元气提升359点": {
            "id": 16014,
            "name": "风语·裤·铸（元气）",
            "score": 2877,
            "attributes": {
                "spunk_base": 359
            }
        },
        "风语·裤·铸（身法） 身法提升359点": {
            "id": 16013,
            "name": "风语·裤·铸（身法）",
            "score": 2877,
            "attributes": {
                "agility_base": 359
            }
        },
        "风语·裤·甲（无双） 无双等级提升1640点": {
            "id": 16270,
            "name": "风语·裤·甲（无双）",
            "score": 1681,
            "attributes": {
                "strain_base": 1640
            }
        },
        "风语·裤·甲（会心） 全会心提升1640点": {
            "id": 16269,
            "name": "风语·裤·甲（会心）",
            "score": 1681,
            "attributes": {
                "all_critical_strike_base": 1640
            }
        },
        "风语·裤·甲（内破） 内功破防等级提升1640点": {
            "id": 16268,
            "name": "风语·裤·甲（内破）",
            "score": 1681,
            "attributes": {
                "magical_overcome_base": 1640
            }
        },
        "风语·裤·甲（外破） 外功破防等级提升1640点": {
            "id": 16267,
            "name": "风语·裤·甲（外破）",
            "score": 1681,
            "attributes": {
                "physical_overcome_base": 1640
            }
        },
        "风语·裤·甲（根骨） 根骨提升210点": {
            "id": 16265,
            "name": "风语·裤·甲（根骨）",
            "score": 1681,
            "attributes": {
                "spirit_base": 210
            }
        },
        "风语·裤·甲（力道） 力道提升210点": {
            "id": 16264,
            "name": "风语·裤·甲（力道）",
            "score": 1681,
            "attributes": {
                "strength_base": 210
            }
        },
        "风语·裤·甲（元气） 元气提升210点": {
            "id": 16263,
            "name": "风语·裤·甲（元气）",
            "score": 1681,
            "attributes": {
                "spunk_base": 210
            }
        },
        "风语·裤·甲（身法） 身法提升210点": {
            "id": 16262,
            "name": "风语·裤·甲（身法）",
            "score": 1681,
            "attributes": {
                "agility_base": 210
            }
        },
        "风语·裤·甲（无双） 无双等级提升1401点": {
            "id": 16044,
            "name": "风语·裤·甲（无双）",
            "score": 1436,
            "attributes": {
                "strain_base": 1401
            }
        },
        "风语·裤·甲（会心） 全会心提升1401点": {
            "id": 16043,
            "name": "风语·裤·甲（会心）",
            "score": 1436,
            "attributes": {
                "all_critical_strike_base": 1401
            }
        },
        "风语·裤·甲（内破） 内功破防等级提升1401点": {
            "id": 16042,
            "name": "风语·裤·甲（内破）",
            "score": 1436,
            "attributes": {
                "magical_overcome_base": 1401
            }
        },
        "风语·裤·甲（外破） 外功破防等级提升1401点": {
            "id": 16041,
            "name": "风语·裤·甲（外破）",
            "score": 1436,
            "attributes": {
                "physical_overcome_base": 1401
            }
        },
        "风语·裤·甲（根骨） 根骨提升180点": {
            "id": 16039,
            "name": "风语·裤·甲（根骨）",
            "score": 1436,
            "attributes": {
                "spirit_base": 180
            }
        },
        "风语·裤·甲（力道） 力道提升180点": {
            "id": 16038,
            "name": "风语·裤·甲（力道）",
            "score": 1436,
            "attributes": {
                "strength_base": 180
            }
        },
        "风语·裤·甲（元气） 元气提升180点": {
            "id": 16037,
            "name": "风语·裤·甲（元气）",
            "score": 1436,
            "attributes": {
                "spunk_base": 180
            }
        },
        "风语·裤·甲（身法） 身法提升180点": {
            "id": 16036,
            "name": "风语·裤·甲（身法）",
            "score": 1436,
            "attributes": {
                "agility_base": 180
            }
        },
        "远风·裤·无双 下装无双等级提高1312": {
            "id": 16345,
            "name": "远风·裤·无双",
            "score": 1341,
            "attributes": {
                "strain_base": 1312
            }
        },
        "远风·裤·内破 下装内功破防等级提高1312": {
            "id": 16344,
            "name": "远风·裤·内破",
            "score": 1341,
            "attributes": {
                "magical_overcome_base": 1312
            }
        },
        "远风·裤·外破 下装外功破防等级提高1312": {
            "id": 16343,
            "name": "远风·裤·外破",
            "score": 1341,
            "attributes": {
                "physical_overcome_base": 1312
            }
        },
        "远风·裤·会心 下装全会心等级提高1312": {
            "id": 16340,
            "name": "远风·裤·会心",
            "score": 1341,
            "attributes": {
                "all_critical_strike_base": 1312
            }
        },
        "远风·裤·无双 下装无双等级提高1120": {
            "id": 16119,
            "name": "远风·裤·无双",
            "score": 1146,
            "attributes": {
                "strain_base": 1120
            }
        },
        "远风·裤·内破 下装内功破防等级提高1120": {
            "id": 16118,
            "name": "远风·裤·内破",
            "score": 1146,
            "attributes": {
                "magical_overcome_base": 1120
            }
        },
        "远风·裤·外破 下装外功破防等级提高1120": {
            "id": 16117,
            "name": "远风·裤·外破",
            "score": 1146,
            "attributes": {
                "physical_overcome_base": 1120
            }
        },
        "远风·裤·会心 下装全会心等级提高1120": {
            "id": 16114,
            "name": "远风·裤·会心",
            "score": 1146,
            "attributes": {
                "all_critical_strike_base": 1120
            }
        }
    },
    "primary_weapon": {
        "风语·兵·铸（武伤） 武器伤害提升1344点": {
            "id": 16237,
            "name": "风语·兵·铸（武伤）",
            "score": 3368,
            "attributes": {
                "weapon_damage_base": 1344
            }
        },
        "风语·兵·铸（疗伤） 疗伤成效提升1592点": {
            "id": 16236,
            "name": "风语·兵·铸（疗伤）",
            "score": 3368,
            "attributes": {
                "therapy_power_base": 1592
            }
        },
        "风语·兵·铸（内攻） 内功攻击提升994点": {
            "id": 16235,
            "name": "风语·兵·铸（内攻）",
            "score": 3368,
            "attributes": {
                "magical_attack_power_base": 994
            }
        },
        "风语·兵·铸（外攻） 外功攻击提升891点": {
            "id": 16234,
            "name": "风语·兵·铸（外攻）",
            "score": 3368,
            "attributes": {
                "physical_attack_power_base": 891
            }
        },
        "风语·兵·铸（武伤） 武器伤害提升1148点": {
            "id": 16011,
            "name": "风语·兵·铸（武伤）",
            "score": 2877,
            "attributes": {
                "weapon_damage_base": 1148
            }
        },
        "风语·兵·铸（疗伤） 疗伤成效提升1360点": {
            "id": 16010,
            "name": "风语·兵·铸（疗伤）",
            "score": 2877,
            "attributes": {
                "therapy_power_base": 1360
            }
        },
        "风语·兵·铸（内攻） 内功攻击提升849点": {
            "id": 16009,
            "name": "风语·兵·铸（内攻）",
            "score": 2877,
            "attributes": {
                "magical_attack_power_base": 849
            }
        },
        "风语·兵·铸（外攻） 外功攻击提升761点": {
            "id": 16008,
            "name": "风语·兵·铸（外攻）",
            "score": 2877,
            "attributes": {
                "physical_attack_power_base": 761
            }
        },
        "风语·兵·甲（武伤） 武器伤害提升672点": {
            "id": 16288,
            "name": "风语·兵·甲（武伤）",
            "score": 1681,
            "attributes": {
                "weapon_damage_base": 672
            }
        },
        "风语·兵·甲（疗伤） 疗伤成效提升796点": {
            "id": 16287,
            "name": "风语·兵·甲（疗伤）",
            "score": 1681,
            "attributes": {
                "therapy_power_base": 796
            }
        },
        "风语·兵·甲（内攻） 内功攻击提升497点": {
            "id": 16286,
            "name": "风语·兵·甲（内攻）",
            "score": 1681,
            "attributes": {
                "magical_attack_power_base": 497
            }
        },
        "风语·兵·甲（外攻） 外功攻击提升446点": {
            "id": 16285,
            "name": "风语·兵·甲（外攻）",
            "score": 1681,
            "attributes": {
                "physical_attack_power_base": 446
            }
        },
        "风语·兵·甲（武伤） 武器伤害提升574点": {
            "id": 16062,
            "name": "风语·兵·甲（武伤）",
            "score": 1436,
            "attributes": {
                "weapon_damage_base": 574
            }
        },
        "风语·兵·甲（疗伤） 疗伤成效提升680点": {
            "id": 16061,
            "name": "风语·兵·甲（疗伤）",
            "score": 1436,
            "attributes": {
                "therapy_power_base": 680
            }
        },
        "风语·兵·甲（内攻） 内功攻击提升424点": {
            "id": 16060,
            "name": "风语·兵·甲（内攻）",
            "score": 1436,
            "attributes": {
                "magical_attack_power_base": 424
            }
        },
        "风语·兵·甲（外攻） 外功攻击提升381点": {
            "id": 16059,
            "name": "风语·兵·甲（外攻）",
            "score": 1436,
            "attributes": {
                "physical_attack_power_base": 381
            }
        }
    }
}
