GAINS: dict[int, dict] = {
    # dps wrist
    40788: dict(skills=[40789]),
    # tank wrist
    42895: dict(buffs=[32019], skills=[42898]),
    41062: dict(skills=[41069]),

    # dps bottom
    40793: dict(buffs=[30748]),
    40794: dict(buffs=[30749, 30770]),
    # tank bottom
    41064: dict(buffs=[30948], skills=[41073]),

    # dps belt
    40791: dict(buffs=[30743]),
    40790: dict(buffs=[30742]),
    42767: dict(skills=[42837]),
    # tank belt
    41063: dict(buffs=[30946]),

    # dps ring
    40804: dict(buffs=[30757]),
    40802: dict(buffs=[30755]),
    40803: dict(buffs=[30756]),
    # tank ring
    41065: dict(buffs=[30950]),

    # hat
    38934: dict(buffs=[29519]),

    # shoes
    38939: dict(buffs=[29524]),
    38944: dict(buffs=[29526]),

    # necklace
    38945: dict(buffs=[29528]),
    38946: dict(buffs=[29529]),
    # pendant
    38949: dict(buffs=[29537]),
    38948: dict(buffs=[29536]),
    # tertiary_weapon
    38950: dict(skills=[38966]),

    # divine
    39088: dict(buffs=[29608]),
    # special weapon
    21319: dict(skills=[23104, 23107, 23177]),
    22035: dict(skills=[21933]),
    43807: dict(buffs=[32709, 32664, 32666], dots={32699: [43758]},
                skills=[42801, 42841, 42817, 42840, 42842, 42839] +
                       [42800, 42844, 42846, 42848, 42845, 42849] +
                       [42711, 43757, 43758]),
    43808: dict(buffs=[32549, 32664, 32666], dots={32699: [43758]},
                skills=[42802, 43720, 43721, 43722, 43724, 43901, 43902] +
                       [42803, 43756] +
                       [42711, 43757, 43758]),

    # element
    4877: dict(buffs=[4761]),
    38578: dict(buffs=[29268]),
    38786: dict(skills=[38787]),
    # enchant
    22169: dict(buffs=[15455]),
    38984: dict(skills=[37562]),
    38985: dict(skills=[37561]),
    22151: dict(skills=[22151]),
    10106: dict(buffs=[15436]),

    22129: {},
    33249: dict(buffs=[24767]),
    22128: {},
    22122: {},
    40512: {}

}
