...
def get_base_attrs(row):
    ...
    for i in range(MAX_BASE_ATTR):
        ...
        attrs.append({'attr': attr, 'attr_type': attr_type, 'value': row[f'Base{i + 1}Max'], 'desc': get_attr_desc(attr)})
...
def get_magic_attrs(row):
    ...
    for i in range(MAX_MAGIC_ATTR):
        ...
        attrs.append({'attr': attr, 'attr_type': attr_type, 'value': value, 'desc': desc})
    return attrs
...
def get_embed_attrs(row):
    ...
    for i in range(MAX_EMBED_ATTR):
        ...
        cap_attr = camel_to_capital(attr[2:])
        attr_type = ATTRIBUTE_TYPE[cap_attr]  # noqa
        attrs.append({'attr': attr, 'attr_type': attr_type, 'value': attr_row['Param1Max'], 'desc': get_attr_desc(attr)})
    return attrs
        ...
        for j in range(MAX_SET_ATTR):
            if attr_id := set_row[f'{i + 1}_{j + 1}']:
                ...
                if attr_type == ATTRIBUTE_TYPE.SKILL_EVENT_HANDLER:
                    ...
                    desc, value = get_attr_desc(attr), param_1
                attrs[i + 1].append({'attr': attr, 'attr_type': attr_type, 'value': value, 'desc': desc})
    return set_id, attrs
...
def get_equip_detail(row):
    detail = {
        ...
        'usage': USAGE_MAP[row['EquipUsage']],
        'level': int(row['Level']),
        'score': int(row['Score']),
...
def get_enchant_attrs(row):
    ...
    for i in range(MAX_ENCHANT_ATTR):
        ...
        attrs.append({'attr': attr, 'attr_type': attr_type, 'value': int(value) if value.isdigit() else value, 'desc': get_attr_desc(attr)})
    return attrs
...
def get_enchant_detail(row):
    detail = {
        ...
        'score': int(row['Score']),
        'attributes': get_enchant_attrs(row),
        'temporary': bool(row['Time']),
    }
    return detail
...
def get_stone_attrs(row):
    ...
    for i in range(MAX_STONE_ATTR):
        ...
        attr_type = ATTRIBUTE_TYPE[cap_attr]  # noqa
        value = row[f'Attribute{i + 1}Value1']
        attrs.append({'attr': attr, 'attr_type': attr_type, 'value': int(value), 'desc': get_attr_desc(attr)})
    return attrs
...
def get_stone_detail(row):
    ...
    if not level:
        return {}
    detail = {'enchant_id': row['ID'], 'item_ids': item_ids, 'name': row['Name'], 'level': level, 'attributes': get_stone_attrs(row)}
    return detail
