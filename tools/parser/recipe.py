...
def set_recipe_to_skill(engine: Engine, recipe, skills: dict[int, dict[int, dict[int, Skill]]]):
    buff_recipes = set()
...
def set_recipe_to_buffs(buff_recipe: BuffRecipe, dots: dict[int, dict[int, dict[int, Dot]]]):
...
def parse_recipe(
        recipe: Recipe, skills: dict[int, dict[int, dict[int, Skill]]], dots: dict[int, dict[int, dict[int, Dot]]]
):
    ...
    for category, dot_ids in dots.items():
        for dot_id, dot_levels in dot_ids.items():
            for dot_level, dot in dot_levels.items():
                buff_recipes |= set_recipe_to_skill(engine, recipe, {dot.buff_id: dot.skills})
